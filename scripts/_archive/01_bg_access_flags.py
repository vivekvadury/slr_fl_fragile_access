...
OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "access"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute exploratory block-centroid access flags across SLR scenarios."
    )
    parser.add_argument(
        "--max-blocks",
        "--max-bg",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional subset of SLR feet to run, e.g. --slr-ft 1 3 6.",
    )
    parser.add_argument(
        "--output-suffix",
        default="",
    ...
    for row in roads.itertuples(index=False):
        for line in iter_lines(row.geometry):
            ...
            if projected_line.length == 0:
                continue
            u = get_node_id(projected_coords[0][0], projected_coords[0][1])
            v = get_node_id(projected_coords[-1][0], projected_coords[-1][1])
            if u == v:
                continue
            edge_records.append(
                {
...
def capped_local_edge_connectivity(
    graph: nx.Graph,
    origin_node: int,
    candidate_service_nodes: list[int],
    *,
    ...
    if origin_node not in graph:
        return 0
    if graph.degree(origin_node) == 0:
        return 0

    origin_component = component_lookup.get(origin_node)
    if origin_component is None:
        return 0

    ordered_candidates: list[int] = []
    seen: set[int] = set()
    for node_id in candidate_service_nodes:
        if node_id not in seen:
            ordered_candidates.append(node_id)
            seen.add(node_id)
    if nearest_service_node is not None and nearest_service_node not in seen:
        ordered_candidates.append(nearest_service_node)

    max_paths = 0
    for service_node in ordered_candidates:
        if service_node not in graph:
            continue
        if component_lookup.get(service_node) != origin_component:
            continue
        if service_node == origin_node:
            candidate_paths = min(max(int(graph.degree(origin_node)), 1), cap)
        else:
            candidate_paths = local_edge_connectivity(
                graph,
                origin_node,
                service_node,
                cutoff=cap,
            )
        max_paths = max(max_paths, int(candidate_paths))
        if max_paths >= cap:
            return cap
    return min(max_paths, cap)
...
def scenario_results_for_origins(
    *,
    ...
    candidate_service_nodes: dict[int, list[int]],
    ...
    for row in result.itertuples(index=False):
        ...
        if origin_valid and origin_node_id in dry_component_lookup:
            ...
            if reachable_service_count > 0 and not centroid_is_flooded:
                max_edge_disjoint_paths = capped_local_edge_connectivity(
                    graph,
                    origin_node_id,
                    candidate_service_nodes.get(origin_node_id, []),
                    component_lookup=dry_component_lookup,
        ...
        is_isolated = int(
            centroid_is_flooded
            or (not origin_valid)
            or reachable_service_count == 0
        )
        is_redundant = int(is_isolated == 0 and max_edge_disjoint_paths >= 2)
        is_fragile = int(is_isolated == 0 and max_edge_disjoint_paths == 1)
        ...
        records.append(
            {
                ...
                "block_centroid_inundated": int(centroid_is_flooded),
...
def main() -> int:
    ...
    candidate_service_nodes = build_service_node_candidate_map(origins, services, nodes)
    ...
    results = pd.concat(scenario_outputs, ignore_index=True)
